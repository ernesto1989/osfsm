/**
 * Module that handles the Consumer Nodes REST API.
 * 
 * Basic CRUD operations for Consumer Nodes in a scenario.
 ...
 * @requires Service/consumersService
 * @requires Controllers/Templates/utilities
 * Ernesto Cantú
 */
const consumersService = require("../../Service/consumersService")
const utilities = require("../Templates/utilities")

/**
 * Method that returns the list of consumer nodes from a specific scenario.
 * 
...
async function getScenarioNodes(req,res){
    try{    
        ...
        let result = await consumersService.getConsumers(scenarioId,regionId);
...
/**
 * Endpoint that handles Consumer Nodes table modifications.
 * 
 * Changes can be:
...
async function saveNode(req,res){
    try{
        ...
        if(!node.new_record){
            //update
            result = await consumersService.updateConsumer(node,region_id);  
            res.status(200);
            res.json({
                "status"  : "success",
                "total"   : result.length,
            ...
            //insert
            result = await consumersService.insertConsumer(node,region_id);
            res.status(200);
            res.json({
                "status"  : "success",
...
/**
 * Method that handles consumer nodes delete
 * @param {*} req 
 * @param {*} res 
 * @returns 
 */
async function deleteNode(req,res){
    try{
        ...
        let node = req.body;
        const region_id = session.region_id;
        let result = await consumersService.deleteConsumer(node,region_id);
